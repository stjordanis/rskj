...
public class BlockHashesHelper {
    ...
    public static boolean isRskipUnitrie(long number) {
        return false; //return SystemProperties.DONOTUSE_blockchainConfig.getConfigForBlock(number).isRskipUnitrie();
    }

    public static byte[] calcTxTrie(List<Transaction> transactions, boolean isRskipUnitrieEnabled) {
        return BlockHashesHelper.getTxTrieRoot(transactions, isRskipUnitrieEnabled);
    }

    public static byte[] calcReceiptsTrie(List<TransactionReceipt> receipts, boolean isRskipUnitrieEnabled) {
        return BlockHashesHelper.calculateReceiptsTrieRoot(receipts, isRskipUnitrieEnabled);
    }
