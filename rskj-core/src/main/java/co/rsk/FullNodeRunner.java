...
public class FullNodeRunner implements NodeRunner {
    ...
    public void run() throws Exception {
        ...
        if (rskSystemProperties.isBlocksEnabled()) {
            setupRecorder(rskSystemProperties.blocksRecorder());
            setupPlayer(rsk, channelManager, blockchain, rskSystemProperties.blocksPlayerPath(), rskSystemProperties.blocksPlayerFormat(), true);
        }
    ...
    private BlockPlayer createBlockPlayer(String format, String blocksPlayerFileName) {
        if ((format==null) || (format.toLowerCase().equals("file"))) {
            return new FileBlockPlayer(rskSystemProperties,blocksPlayerFileName);
        } else
        if (format.toLowerCase().equals("blockstore")) {
            return new BlockstoreBlockPlayer(rskSystemProperties, blocksPlayerFileName);
        } else
            return null;
    }
